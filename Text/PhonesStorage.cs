...
namespace Text
{
    class PhonesStorage
    {
        private readonly string DbFilePath = @"C:\Users\User\Desktop\PhoneDbFolder\phonedb.csv";
            ...
            
        {
            ...
            for (int i = 0; i < records.Length; i++)
            {
                string? record = records[i];
                var desirializedRecord = DeserializeRecord(record);
                list.Add(desirializedRecord);
            }
            return list;
        }


        public void SearchByLastAndFirstName()
        {
            Console.WriteLine("Write element");
            var input = Console.ReadLine();

            List<PhoneRecord> rec = GetAll();
            foreach(var i in rec)
            {
                if (i.FirstName == input)
                    Print(i);
                else if (i.LastName == input)
                    Print(i);
                else if (i.PhoneNumber == input)
                    Print(i);
            }            
        }
